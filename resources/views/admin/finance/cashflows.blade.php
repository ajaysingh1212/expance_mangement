    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table mb-0">
                <thead><tr><th>Title</th><th>Source</th><th>Bank</th><th>Expected</th><th>Date</th><th>Status</th><th>Attachment</th><th></th></tr></thead>
                <tbody>
                @forelse($cashflows as $cashflow)
                <tr>
                    <td><strong>{{ $cashflow->title }}</strong><div class="text-muted small">{{ $cashflow->notes }}</div></td>
                    <td>{{ $cashflow->ledger?->name ?: 'Direct' }}</td>
                    ...
                    <td class="text-right">@can('finance.approve')@if(!in_array($cashflow->status, ['received', 'approved']))<form action="{{ route('admin.finance.cashflows.approve', $cashflow) }}" method="POST">@csrf<button class="btn btn-sm btn-success"><i class="fas fa-check"></i></button></form>@endif@endcan</td>
                </tr>
                @empty
